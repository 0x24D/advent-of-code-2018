...
int main() {
    ...
    if (inputFile.is_open()) {
        ...
        while (inputFile >> inches >> inches >> inches >> rect) {
            ...
            int topInches = std::atoi(
                inches.substr(commaDelimiter + 1, colonDelimiter - commaDelimiter - 1).c_str());
        ...
        std::cerr
            << "Couldn't find input file, expected to run from root directory: e.g. ./bin/day-3\n";
        return 1;
    }

    for (int i = 0; i < fabric.size(); ++i) {
        numberOfClaims += (std::count_if(fabric[i].begin(), fabric[i].end(), [](int n) {
            return n == -1;
        }));
    }
