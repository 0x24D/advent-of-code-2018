...
int main() {
    ...
    if (inputFile.is_open()) {
        ...
        std::cerr
            << "Couldn't find input file, expected to run from root directory: e.g. ./bin/day-4\n";
        return 1;
    }
    ...
    auto mostAsleep = std::max_element(
        guardsAsleep.begin(), guardsAsleep.end(), [](std::pair<int, int> a, std::pair<int, int> b) {
            return a.second < b.second;
        });

    auto guardRecord = std::find_if(timesAsleep.begin(), timesAsleep.end(), [&mostAsleep](auto p) {
        return p.first == mostAsleep->first;
    });

    if (guardRecord != timesAsleep.end()) {
        auto maxEl = std::max_element(guardRecord->second.begin(), guardRecord->second.end());
        for (int i = 0; i < guardRecord->second.size(); ++i) {
            if (guardRecord->second[i] == *maxEl) {
                std::cout << (mostAsleep->first) << " * " << i << " = " << (mostAsleep->first * i)
                          << "\n";
                break;
            }
        }
    ...
    auto maxEl = std::max_element(guardsAsleep.begin(), guardsAsleep.end(), [](auto a, auto b) {
        return a.second < b.second;
    });
    guardRecord = std::find_if(timesAsleep.begin(), timesAsleep.end(), [&maxEl](auto p) {
        return p.first == maxEl->first;
    });
